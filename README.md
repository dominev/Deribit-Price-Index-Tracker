...
| Тестирование        | pytest (Тесты удалены из репозитория для переписывания)|
...
- **Graceful shutdown**: корректное закрытие соединений (`aiohttp`, PostgreSQL) через `lifespan` FastAPI.
- **Rate limiting**: `asyncio.Semaphore` для ограничения запросов к Deribit API (5 запросов/секунду).
- **Retry-логика**: декоратор `tenacity` с экспоненциальной задержкой для обработки сетевых ошибок.
...
- **Контейнеризация**: все сервисы (app, worker, beat, db, redis) оркестрируются через `docker-compose`.
...
## Тестирование
В проекте есть начальные тесты, но они не обязательны для проверки.
