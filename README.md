...
- **Логика задержки**: декоратор `tenacity` с задержкой для обработки сетевых ошибок.
...
| `DERIBIT_API_URL`  | Deribit API                          | `https://www.deribit.com/api/v2/public`       |
...
- `offset` (optional) — смещение для пагинации (разделения)
